...
#include "tuiFnctRealCplx3McuUsart.h"

// ****************************************************
// section start **** CONSTRUCTOR *****

    tuiFnctRealCplx3McuUsart_t::tuiFnctRealCplx3McuUsart_t      ([[maybe_unused]] pphUsart_t* p_pPphUSART    )   :
...
// section start **** PROCESS MANAGEMENT *****
    ...
    // protected:
        bool tuiFnctRealCplx3McuUsart_t::init   	            (dtyIntfcAbstractUnitGraphic_t* p_this)  {
            tuiFnctRealUnit2Cntnr_t::init(p_this);
            g_pPphUsartBuff = g_pPphUsart->rxBuff_getPtr();
            return true;
        }

        bool tuiFnctRealCplx3McuUsart_t::loop   	            ([[maybe_unused]] dtyIntfcAbstractUnitGraphic_t* p_this)  {
...
// section start **** SPECIFIC DATA *****
    ...
    // private:
        void tuiFnctRealCplx3McuUsart_t::sendMessage    (dtyIntfcAbstractUnitFunctional_t* p_this)     {

            static_cast<tuiFnctRealCplx3McuUsart_t*>(p_this)->g_pPphUsart->txBuff_send(
                 reinterpret_cast<uint8_t*>((static_cast<tuiFnctRealCplx3McuUsart_t*>(p_this)->g_sendingDataBuffer))
                ,20
                );
                        static_cast<tuiFnctRealCplx3McuUsart_t*>(p_this)->g_recevingDataCntnr.add(
                             reinterpret_cast<uint8_t*>((static_cast<tuiFnctRealCplx3McuUsart_t*>(p_this)->g_sendingDataBuffer))
                            ,20
                            ,kMarker_t::deviceTx
                            ,kDataType_t::ascii
                        ...
                        static_cast<tuiFnctRealCplx3McuUsart_t*>(p_this)->g_recevingDataCntnr.selectElementLast();
...
// section start **** DEBUG FUNCTIONS *****
    ...
    void tuiFnctRealCplx3McuUsart_t::dbgSet                ([[maybe_unused]] int32_t p_value)   {
        // g_pMcuPinPort->pinRead(0,p_value);
    }

    void tuiFnctRealCplx3McuUsart_t::dbgSetMode            (bool p_select)   {
        g_dbgSelect = p_select;
    }

    void tuiFnctRealCplx3McuUsart_t::dbgRefreshValue       (dtyIntfcAbstractUnitGraphic_t* p_this)      {
