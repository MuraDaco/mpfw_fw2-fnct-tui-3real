...
// section start **** CONSTRUCTOR *****
    ...
    tuiFnctRealCplx3McuUsart_t::tuiFnctRealCplx3McuUsart_t      ([[maybe_unused]] pphUsartRs485_t* p_pPphUSART    )   :
...
// section start **** SPECIFIC DATA *****
    ...
    // private:
        void tuiFnctRealCplx3McuUsart_t::sendMessage    (dtyIntfcAbstractUnitFunctional_t* p_this)     {
            tuiFnctRealCplx3McuUsart_t* l_this = static_cast<tuiFnctRealCplx3McuUsart_t*>(p_this);
            uint8_t* l_ptrBuf = reinterpret_cast<uint8_t*>(l_this->g_sendingDataBuffer);

            if( l_this->g_pPphUsart->tx_packetSent(l_ptrBuf, sizeof(g_sendingDataBuffer)) )   {
                uint8_t l_bufferLenghth = l_this->g_pPphUsart->getBufferLength();
